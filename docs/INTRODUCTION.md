...
화면은 작업 선택 버튼, 실행 명령어 방법, 매개변수 입력 폼, Preview/실행 버튼으로 구성합니다. 실행 시에는 확인 문구 `RUN` 입력이 필요합니다. 스크립트 기준은 Oracle Analytics Server 공식 문서 [About the Scripts for Managing Service Instances](https://docs.oracle.com/en/middleware/bi/analytics-server/administer-oas/scripts-managing-service-instances.html)를 참고합니다.
